...
RuleLoader.prototype.initializeRules = function(reload) {
  var loader  = this;
  ...
  try {
    ...
    loader.logger.warn(err, 'Fatal error reading rules directory');
    return false;
  }

  // Watch rules directory for changes.
  if (! reload) {
    fs.watch(dir, function(ev, filename) {
      if (ev !== 'change') {
        return;
      }
      loader.logger.info("File '%s' changed, reloading configuration", filename);
      loader.initializeRules(true);
    });
  }

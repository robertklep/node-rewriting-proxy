...
module.exports = function(proxy) {
  return function(req, res, next) {
    var matches = req.matchingRules.modify || [];
    ...
    if (matches.length) {
      ...
      req.pipe(http.request({
        ...
        if (
            ...
            (httpResponse.headers['content-type'] || '').indexOf('text/') === -1
        ) {
        ...
        httpResponse.on('data', function(chunk) {
          ...
          switch(httpResponse.headers['content-encoding']) {
            ...
            default       : doModify(null, body)          ; break;
          }
...
function transformResponse(proxy, rule, req, res, headers, body) {
  ...
  var sandbox = {
    ...
    gb        : {
      request           : req,
      response          : res,
      addStyleElement   : function(css) {
        var content = '<style>' + css + '</style>';
        body = body.replace(/<\s*\/\s*head.*?>/i, content + '$&');
        return body;
      },
  ...
  try {
    ...
    proxy.logger.error({ err : err, rule : rule }, 'transform execution error');
  }
  return body;
}

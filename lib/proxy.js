...
var AdBlockProxy = function AdBlockProxy(options) {
  ...
  // Initialize rule set.
  this.rulesPath  = __dirname + '/../rules';
  this.rulesFiles = [];
  this.rules      = [];
...
AdBlockProxy.prototype.initializeRules = function(reload) {
  ...
  var files = fs.readdirSync(this.rulesPath).filter(function(file) {
...
AdBlockProxy.prototype.run = function() {
  this.logger.info('starting proxy on http://127.0.0.1:%s/', this.options.port);
  // XXX: hoxy doesn't allow us to check if the bind went okay...
  this.proxy.listen(this.options.port);
};
